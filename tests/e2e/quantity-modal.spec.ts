...
test.describe('US-002: Quantity Modal Management', () => {
  ...
  test('should open quantity modal when clicking a color card', async ({ page }) => {
  ...
  test('should handle direct quantity input in the input field', async ({ page }) => {
    const testColor = TEST_COLORS.RV_252;
  ...
  test('should cancel changes when clicking Cancel button', async ({ page }) => {
    const testColor = TEST_COLORS.RV_252;
    const initialQuantity = SAMPLE_INVENTORY[testColor];
  ...
  test('should handle keyboard navigation (Escape to close, Enter to save)', async ({ page }) => {
    const testColor = TEST_COLORS.RV_252;
  ...
  test('should test complete workflow scenarios', async ({ page }) => {
    for (const scenario of QUANTITY_SCENARIOS.slice(0, 4)) { // Test first 4 scenarios
      await test.step(`Test scenario: ${scenario.description}`, async () => {
        ...
        if (scenario.change > 0) {
          ...
          await quantityModalPage.decrementQuantityBy(Math.abs(scenario.change));
        }
